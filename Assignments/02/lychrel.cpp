...
int main() {
    ...
    for (int id = 0; id < MaxThreads; ++id) {
        threads[id] = std::thread { [&]() {
            ...
            while (data.getNext(std::max(data.available()/3000, static_cast<size_t>(1)), number)) {
                for (int i = 0; i < number.size(); ++i) {
                    ...
                    //   (MaxIterations) for a particular number.  If we're not less
                    //   tha the current maximum (maxIter) or we've exceeded the number
                    //   of permissible iterations, update our records, which possibly means
                    //   discarding our current maximum and rebuilding our records list.
                    if (!(iter < maxIter || iter == MaxIterations)) {
                        Record record{number[i], n};
